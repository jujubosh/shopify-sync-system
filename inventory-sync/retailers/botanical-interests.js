...
// Configuration
const CONFIG_PATH = path.join(__dirname, '../../config/retailers/botanical-interests-config.json');
...
function log(message, level = 'info') {
    ...
    // Write to log file
    const logFile = path.join(LOG_DIR, `inventory-sync-botanical-interests-${new Date().toISOString().split('T')[0]}.log`);
...
async function main(args) {
    ...
    try {
        log('Starting Botanical Interests inventory sync...');
        ...
        if (!targetStoreConfig.accessToken) {
            throw new Error('Missing target store API token (SHOPIFY_BOTANICALINTERESTS_TOKEN or database token)');
        }
            ...
            if (globalConfig) {
                ...
                const emailResults = {
                    inventory: {
                        ...
                        details: {
                            successfulUpdates: (results.successful || results.updated || 0) > 0 ? [{ retailer: 'Botanical Interests', sku: 'Multiple SKUs' }] : [],
                            locationMismatches: locationMismatches > 0 ? [{ retailer: 'Botanical Interests', sku: 'Location mapping issues', count: locationMismatches }] : [],
                            failures: results.errors.map(error => ({
                                retailer: 'Botanical Interests',
        ...
        return {
            statusCode: 200,
            body: {
                message: 'Botanical Interests inventory sync completed',
                results,
                duration
            }
        };
        ...
        return {
            statusCode: 500,
            body: {
                message: 'Error during Botanical Interests inventory sync',
                error: error.message,
                duration
            }
        };
