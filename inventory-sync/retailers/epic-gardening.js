...
async function loadRetailerConfig() {
    try {
        ...
        const retailer = await retailerService.loadRetailerById('epic-gardening');
...
function log(message, level = 'info') {
    ...
    // Write to log file
    const logFile = path.join(LOG_DIR, `inventory-sync-epic-gardening-${new Date().toISOString().split('T')[0]}.log`);
...
async function main(args) {
    ...
    try {
        log('Starting Epic Gardening inventory sync...');
        ...
        if (!targetStoreConfig.accessToken) {
            throw new Error('Missing target store API token (SHOPIFY_EPICGARDENING_TOKEN or database token)');
        }
            ...
            if (globalConfig) {
                ...
                const emailResults = {
                    inventory: {
                        ...
                        details: {
                            successfulUpdates: (results.successful || results.updated || 0) > 0 ? [{ retailer: 'Epic Gardening', sku: 'Multiple SKUs' }] : [],
                            locationMismatches: locationMismatches > 0 ? [{ retailer: 'Epic Gardening', sku: 'Location mapping issues', count: locationMismatches }] : [],
                            failures: results.errors.map(error => ({
                                retailer: 'Epic Gardening',
        ...
        return {
            statusCode: 200,
            body: {
                message: 'Epic Gardening inventory sync completed',
                results,
                duration
            }
        };
        ...
        return {
            statusCode: 500,
            body: {
                message: 'Error during Epic Gardening inventory sync',
                error: error.message,
                duration
            }
        };
